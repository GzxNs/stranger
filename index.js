...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5596984081026:+55 96 98408-1026\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
const config = {
    XBOT: '𝐆𝐀𝐀𝐑𝐀𝐙𝐈𝐍𝐇𝐎 愛', 
    instagram: 'https://www.instagram.com/_gaaragzx_?r=nametag', 
    nomer: 'wa.me/559684081026',
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá🤠 @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* siga as regras pra n levar ban😳🤝`
				...
				teks = `sayonara pessoa @${num.split('@')[0]} não volte mais😔✌`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["559684081026@s.whatsapp.net"] 
			...
			switch(command) {
				...
				case 'ping':    
                    ...
                    client.updatePresence(from, Presence.composing) 
				    uptime = process.uptime()
                    nzwa.sendMessage(from, `*Velocidade: ${latensi.toFixed(4)} _Segundos_*\n*Dispositivo: A71 SAMSUNG*\n*RAM: 8/128*\n*Rede: WA-FI*
                    \n*Status: Normal*`, text, { quoted: mek})
                    break
                 case 'pinterest':
                     tels = body.slice(11)
...
case 'lofi':
					...
					break
			    case 'loli':
                     gatauda = body.slice(6)                                       reply(mess.wait)
                ...
                case 'playmp3':
                    ...
                    infomp3 = `「 *TIMELINE PLAY MP3* 」\n*• Título:* ${anu.result.title}\n*• Link:* ${anu.result.source}\n*• Tamanho:* ${anu.result.size}\n\n*espere um tempo para envia este comando dnv, nao spam!*`
				...
				case 'tulis':
				    client.updatePresence(from, Presence.composing)
			        if (args.length < 1) return reply(`O que você quer que eu escreva?`)
			        reply(mess.wait)
					tulis = body.slice(7)
				    nama = tulis.split("/")[0];
					kelas = tulis.split("/")[1];
			...
			case 'mapa': 
				...
				case 'audio':
					...
					if (args.length < 2) return client.sendMessage(from, 'qual txt vc quer q eu fale affe🙄?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
				...
				case 'sigla':
					...
					break 	
				case 'hilih': 
					if (args.length < 1) return reply('qual txt deseja 🙄?')
            ...
            case 'creator':
            case 'gaarazinho':
                  client.sendMessage(from, {displayname: "Gaara adm", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'toimg':
				case 'converter':
					if (!isQuotedSticker) return reply('marca a fig 🙄')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('algo deu erro ;(, tente dnovo dps ^_^')
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('porno ativado 🙄')
						nsfw.push(from)
				...
				case 'boasvindas':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('boas vindas ativado !!!')
						welkom.push(from)
				...
				case 'clonar':
					...
					try {
						...
						mentions(`Perfil atualizado com sucesso usando a foto do perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
